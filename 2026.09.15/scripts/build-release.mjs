...
const files=['index.html','en.html','nia-ontology-workshop-with-worflogy.html','robots.txt','sitemap.xml','CNAME','.nojekyll','lib/phaser.min.js','lib/mammoth.browser.min.js','lib/vis-network.min.js','lib/workshop-safety.js','lib/PHASER-LICENSE.txt','lib/THREE-LICENSE.txt'];
function collect(dir){for(const item of fs.readdirSync(path.join(root,dir),{withFileTypes:true})){const relative=path.join(dir,item.name);if(item.isSymbolicLink())throw new Error('Symbolic links are not allowed: '+relative);if(item.name.startsWith('.'))continue;if(item.isDirectory())collect(relative);else if(/\.(css|js|woff2|png|jpe?g|svg|webp|ico|txt|mp4)$/i.test(item.name))files.push(relative);}}
