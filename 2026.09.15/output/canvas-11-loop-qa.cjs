...
(async()=>{
 ...
 for(const [width,lang]of [[1280,'ko']]){
  ...
  await page.waitForTimeout(500);assert.equal(await scene.getAttribute('data-cycle'),'1','cycle repeats automatically');
  assert(!(await section.locator('[data-ui="next-stage"]').isDisabled()),'loop stays active');
  await section.locator('[data-ui="reset"]').click();
  assert.equal(await scene.getAttribute('data-cycle'),'0');
  assert.equal(await scene.getAttribute('data-stage'),'0');
  console.log('Return-signal synchronization, continuous loop and reset passed.');
  await page.close();
 }
