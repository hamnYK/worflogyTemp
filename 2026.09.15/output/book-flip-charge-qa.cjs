...
assert(Math.abs(await p.evaluate(()=>cam.position.length())-12)<.01);await p.locator('[data-camera="in"]').click();await p.locator('[data-camera="home"]').click();assert(Math.abs(await p.evaluate(()=>cam.position.length())-12)<.01);assert.equal(await p.locator('.chip-power').inputValue(),'1');
await p.mouse.move(pos.x,pos.y);await p.mouse.down();
const samples=[];for(let i=0;i<70;i++){samples.push(Number(await p.locator('.chip-power').inputValue()));await p.waitForTimeout(100);}
const peak=samples.findIndex(v=>v>9.5),trough=samples.findIndex((v,i)=>i>peak&&v<1.5);assert(peak>0);assert(trough>peak);assert(samples.slice(trough+1).some(v=>v>9.5));
await p.waitForTimeout(500);assert((await p.locator('.chip-status').innerText()).includes('Charging'));assert(Number(await p.locator('.chip-power').inputValue())>1);await p.screenshot({path:'output/book-flip-hand-detail.png'});await p.mouse.up();await p.waitForSelector('.book-flip-game[data-phase="moving"]');assert.equal(await p.locator('.book-flip-game').getAttribute('data-hits'),'1');assert.equal(await p.locator('.chip-power').inputValue(),'1');assert(await p.locator('[data-book="hardcover"]').isDisabled());
...
console.log('PASS: repeating rise/fall charge, per-hit reset, zoomed default/home view, pointer charge/release, disabled book switch while moving, fall failure/reset, green-face win and 3-second return, no browser errors.');
