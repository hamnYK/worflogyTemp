...
await p.evaluate(()=>{
const c=window.qaHost.querySelector('canvas'),h=window.qaHost.querySelector('.basket-loft'),start=+h.value;
c.dispatchEvent(new KeyboardEvent('keydown',{key:'ArrowUp',bubbles:true,cancelable:true}));if(+h.value!==start+2)throw Error('Up must raise loft');
c.dispatchEvent(new KeyboardEvent('keydown',{key:'ArrowDown',bubbles:true,cancelable:true}));if(+h.value!==start)throw Error('Down must lower loft');
for(let i=0;i<30;i++)c.dispatchEvent(new KeyboardEvent('keydown',{key:'ArrowUp',bubbles:true,cancelable:true}));if(+h.value!==80)throw Error('Upper clamp');
for(let i=0;i<30;i++)c.dispatchEvent(new KeyboardEvent('keydown',{key:'ArrowDown',bubbles:true,cancelable:true}));if(+h.value!==40)throw Error('Lower clamp');
});
...
await p.waitForSelector('.basketball-game[data-phase="held"]');await p.locator('.chip-power').fill('10.4');await p.locator('.basket-loft').fill('68');await p.evaluate(()=>{
const g=window.qaGame,c=window.qaHost.querySelector('canvas'),r=c.getBoundingClientRect();
function event(type,x,z,buttons){const v=new window.qaThree.Vector3(x,.6,z).project(window.qaCamera);c.dispatchEvent(new PointerEvent(type,{clientX:r.left+(v.x+1)*r.width/2,clientY:r.top+(1-v.y)*r.height/2,button:0,buttons,pointerId:1,pointerType:'mouse',bubbles:true}));}
event('pointerdown',g.p.x,g.p.z,1);event('pointermove',g.p.x+10.4/3,g.p.z,1);
if(g.phase!=='held')throw Error('Drag must not fire early');
event('pointerup',g.p.x+10.4/3,g.p.z,0);
if(g.phase!=='flying')throw Error('Release must fire');
});
if(i<2){await p.waitForSelector('.basketball-game[data-phase="attempt-end"]');assert.equal(await p.locator('.basketball-game').getAttribute('data-total'),String((i+1)*6));await p.waitForSelector('.basketball-game[data-phase="ready"]');}
}
await p.waitForSelector('.basketball-game[data-phase="results"]');assert.equal(await p.locator('.basket-results strong').innerText(),'18 / 18');assert.equal(await p.evaluate(()=>localStorage.getItem('worflogy-basketball-best')),'18');
await p.screenshot({path:'output/basketball-18-points.png'});await p.waitForTimeout(2000);assert.equal(await p.evaluate(()=>window.qaWins),0);await p.waitForFunction(()=>window.qaWins===1,{},{timeout:2000});await p.evaluate(()=>window.qaHandle.dispose());assert.deepEqual(errors,[]);
console.log('PASS: pointer-controlled physical catch, automatic bearing, three rear six-point baskets, best score persistence and 3-second exit.');
