...
await p.evaluate(()=>{
const g=window.qaGame,c=window.qaHost.querySelector('canvas'),r=c.getBoundingClientRect(),loft=window.qaHost.querySelector('.basket-loft');
const v=new window.qaThree.Vector3(g.p.x,.6,g.p.z).project(window.qaCamera);
const event={clientX:r.left+(v.x+1)*r.width/2,clientY:r.top+(1-v.y)*r.height/2,button:0,pointerId:1,pointerType:'mouse',bubbles:true};
const before=window.qaCamera.position.clone();
c.dispatchEvent(new PointerEvent('pointerdown',event));
c.dispatchEvent(new WheelEvent('wheel',{deltaY:-100,bubbles:true,cancelable:true}));
if(+loft.value!==67)throw Error('Wheel up must raise shot angle');
if(!before.equals(window.qaCamera.position))throw Error('Aiming wheel must not zoom camera');
for(let i=0;i<30;i++)c.dispatchEvent(new WheelEvent('wheel',{deltaY:-100,bubbles:true,cancelable:true}));
if(+loft.value!==80)throw Error('Max elevation');
for(let i=0;i<30;i++)c.dispatchEvent(new WheelEvent('wheel',{deltaY:100,bubbles:true,cancelable:true}));
if(+loft.value!==40)throw Error('Min elevation');
c.dispatchEvent(new PointerEvent('pointerup',event));
if(g.phase!=='held')throw Error('Stationary release must not launch');
c.dispatchEvent(new WheelEvent('wheel',{deltaY:100,bubbles:true,cancelable:true}));
if(before.equals(window.qaCamera.position))throw Error('Idle wheel must zoom');
});
