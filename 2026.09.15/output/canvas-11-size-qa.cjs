...
(async()=>{
 ...
 for(const [width,lang]of [[1280,'ko'],[390,'ko'],[320,'en'],[412,'en'],[1280,'en']]){
  ...
  assert(!measure.overflow,'no horizontal overflow');const heights=await page.locator('#section-spatial-9 .diagram-canvas,#section-spatial-10 .diagram-canvas,#section-game-11 .diagram-canvas').evaluateAll(ns=>ns.map(n=>n.getBoundingClientRect().height));assert(heights.every(h=>h===(width>760?620:360)),JSON.stringify(heights));console.log('Matching heights: '+heights.join(', '));
