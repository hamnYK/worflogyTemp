...
(async()=>{
 ...
 try{
 for(const [width,lang]of [[761,'en'],[900,'ko'],[1024,'en']]){
  ...
  const clear=await section.evaluate(s=>{const a=s.querySelector('.canvas-example-card').getBoundingClientRect(),b=s.querySelector('[data-football-graph]').getBoundingClientRect();const r=document.createRange();r.selectNodeContents(s.querySelector('.football-title'));const t=r.getBoundingClientRect();const hit=q=>a.x<q.right&&a.right>q.x&&a.y<q.bottom&&a.bottom>q.y;return !hit(b)&&!hit(t);});assert(clear,'preview must not overlap graph or title');
