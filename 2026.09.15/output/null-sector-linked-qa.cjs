...
(async()=>{
const root=path.join(process.cwd(),'tmp','release-1790341435627');
...
try{const p=await browser.newPage({viewport:{width:1440,height:1000},reducedMotion:'reduce'});const errors=[],bad=[];p.on('pageerror',e=>errors.push(e.message));p.on('response',r=>{if(r.status()>=400&&r.url().includes('gkadudrnr-thegame'))bad.push(r.url());});
await p.goto('http://127.0.0.1:'+server.address().port+'/preview/index.html');await p.locator('.arcade-open').click();await p.locator('.arcade-null-sector-play').click();
...
const game=p.frameLocator('.null-sector-frame');await game.locator('#skip-prologue').waitFor({timeout:60000});assert(await p.locator('.null-sector-load-status').isHidden());
await game.locator('#skip-prologue').click();await game.locator('[data-faction=human]').click();await game.locator('#deploy').click();await game.locator('#viewport canvas').waitFor({timeout:60000});
...
await p.screenshot({path:'output/null-sector-linked-desktop.png'});await p.locator('.null-sector-player-bar button').click();assert.equal(await p.locator('.null-sector-frame').count(),0);
...
await p.setViewportSize({width:390,height:844});assert(await p.locator('.null-sector-player-bar button').isVisible());await p.screenshot({path:'output/null-sector-linked-mobile.png'});await p.locator('.null-sector-player-bar button').click();assert.equal(await p.locator('.null-sector-frame').count(),0);
