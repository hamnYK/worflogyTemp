...
await p.waitForSelector('.eraser-game[data-phase="lost"]');assert((await p.locator('.chip-status').innerText()).includes('ring-out'));
...
await p.evaluate(()=>{g.bodies[0].position.set(0,.451,0);g.bodies[1].position.set(0,.151,0);g.bodies.forEach(b=>{b.quaternion.setFromEuler(0,0,0);b.velocity.setZero();b.angularVelocity.setZero();});g.bodies[0].quaternion.setFromEuler(0,Math.PI/2,0);g.phase='moving';g.still=0;});
