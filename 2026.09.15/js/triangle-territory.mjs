...
export function mountTriangleTerritory(host,{onExit,onWin=onExit,english=false}={}){
 ...
 function preview(i){if(selected===null||selected===i||game.turn!==0||game.invalid(selected,i))return;const count=game.completed(selected,i).length;status.textContent=(selected+1)+'–'+(i+1)+t(' 연결 시: ',' connection: ')+(count?t('내 세모 ','claim ')+count+t('개 획득 · 추가 차례',' triangle(s) and play again'):t('완성되는 세모 없음 · 컴퓨터 차례로 넘어갑니다.','no triangle completed; computer plays next.'));}
 const table=createTriangleTable(board,positions=>{positions.forEach((p,i)=>{if(buttons[i]){buttons[i].style.left=p.x+'%';buttons[i].style.top=p.y+'%';buttons[i].hidden=!p.visible;}});if(buttons[focused]?.hidden){const next=buttons.findIndex(b=>!b.hidden);if(next>=0){focused=next;buttons.forEach((b,i)=>b.tabIndex=i===focused?0:-1);}}});
 if(table){
 ...
 function message(){
  if(game.phase==='finished')return (game.winner===-1?t('무승부!','Draw!'):game.winner===0?t('승리!','You win!'):t('컴퓨터 승리.','Computer wins.'))+' '+share(0)+' : '+share(1)+(game.winner===0?t(' · 3초 후 게임 선택으로 돌아갑니다.',' · Returning to Games in 3 seconds.'):t(' · 3초 후 새 판으로 재도전합니다.',' · Trying a new board in 3 seconds.'));
  if(game.turn===1)return t('컴퓨터가 선을 고르고 있습니다.','Computer is choosing an edge.');
  return selected===null?t('내 차례 · 점 두 개를 연결하세요. 교차 없이 빈 세모를 닫으면 추가 차례. 최종 획득 면적이 크면 승리합니다. 방향키 이동·Enter/Space 선택.','Your turn · Connect two dots. Close an empty triangle without crossing edges for another turn. Largest claimed area wins. Arrow keys move; Enter/Space selects.'):t('점 ','Dot ')+(selected+1)+t(' 선택 · 연결할 점을 고르세요. 같은 점이나 Esc로 취소합니다.',' selected · Choose a second dot. Select the same dot or press Esc to cancel.');
 }
 function render(notice=''){
  root.dataset.phase=game.phase;root.dataset.turn=String(game.turn);root.dataset.edges=String(game.edges.length);
  root.querySelector('.triangle-score-you').textContent=share(0);root.querySelector('.triangle-score-ai').textContent=share(1);
  root.querySelector('.triangle-score').setAttribute('aria-label',t('전체 획득 면적 중 점유율: 나 ','Share of all claimed area: You ')+share(0)+t(', 컴퓨터 ', ', Computer ')+share(1));
  root.querySelector('.triangle-score').title=t('양쪽이 획득한 면적 합계 기준 · 승패는 반올림 전 면적으로 판정','Share of combined claimed area · Winner uses unrounded area');
  ...
  root.querySelector('.triangle-count').textContent=t('획득 면적 기준 · 세모: 나 ','By claimed area · Triangles: You ')+game.scores[0]+t('개 / 컴퓨터 ',' / Computer ')+game.scores[1]+t('개','');
  fills.replaceChildren(...game.triangles.map(({ids,owner})=>svg('polygon',{points:ids.map(i=>`${game.points[i].x},${game.points[i].y}`).join(' '),class:owner===0?'triangle-owned-you':'triangle-owned-ai'})));
  // Owner initials accompany color so claimed land is distinguishable without color.
  for(const {ids,owner} of game.triangles){const center=ids.reduce((p,i)=>({x:p.x+game.points[i].x/3,y:p.y+game.points[i].y/3}),{x:0,y:0});const label=svg('text',{x:center.x,y:center.y,'text-anchor':'middle','dominant-baseline':'central',class:'triangle-owner-label'});label.textContent=owner===0?'Y':'C';fills.append(label);}
 ...
 function queueAI(){
  ...
  const generation=round;timer=setTimeout(()=>{
   ...
   const result=game.play(...move);lastEdge=move;render(result.captured?t('컴퓨터 세모 ','Computer claims ')+result.captured+t('개 획득.',' triangle(s).'):'');
 ...
 function choose(i){
  ...
  if(!result.ok){const reasons={duplicate:t('이미 연결된 선입니다.','That edge already exists.'),'through-point':t('다른 점을 관통할 수 없습니다.','An edge cannot pass through another dot.'),'near-point':t('다른 점에 너무 가깝습니다. 점에 닿거나 스치는 선은 연결할 수 없습니다.','Too close to another dot. Edges must leave a visible gap around other dots.'),crossing:t('기존 선과 교차할 수 없습니다.','Edges cannot cross.')};render(reasons[result.reason]||t('연결할 수 없는 점입니다.','That connection is not allowed.'));return;}
  lastEdge=[selected,i];selected=null;render(result.captured?t('세모 ','Claimed ')+result.captured+(result.finished?t('개 획득!',' triangle(s)!'):t('개 획득! 한 번 더 두세요.',' triangle(s)! Play again.')):'');queueAI();
 }
 function newBoard(){
  clearTimeout(endTimer);endTimer=null;resultOverlay.classList.remove('show');resultOverlay.textContent='';
  clearTimeout(timer);round++;selected=null;focused=0;lastEdge=null;game=new TriangleTerritory({first,clearance:18});root.dataset.clearance=String(game.clearance);root.dataset.first=String(first);first=1-first;
