...
export function footballGuide(g,t,dragging=false){
 ...
 const controls=t(' ←/→ 조준 · ↑/↓ 또는 PageUp/PageDown 힘 조절 · Space 발사.',' Left/right aim · Up/down or PageUp/PageDown power · Space launches.');
 if(g.selected!==null){
 if(g.opening)return t('선택한 칩을 뒤로 당겼다 놓아 펼치세요. 시작 동작은 횟수·충돌 실패에서 제외됩니다.','Pull and release the selected chip to spread the cluster. The opening is uncounted and allows collisions.')+controls;
 return t('칩 ','Chip ')+(g.selected+1)+t(' 선택 고정. ',' locked. ')+(g.canShoot?t('두 칩 사이를 통과해 골을 노리세요.','Shoot through the gap into the goal.'):t('다른 두 칩 사이를 통과시키세요. 칩 충돌은 실패입니다.','Pass between the other two chips. Chip collisions fail.'))+(dragging?t(' 놓으면 발사됩니다.',' Release to launch.'):controls);
 }
...
export function basketballGuide(g,t,{mode,gripperActive,loft=65}={}){
 if(g.phase==='ready')return g.attempt+'/3 · '+(mode==='toss'?t('당기는 거리로 토스 힘을 정하고 놓으세요. 칩은 위로 던져집니다.','Pull farther for a stronger upward toss; release to throw.'):t('칩을 당겼다 놓아 던지세요. PageUp/PageDown 또는 슬라이더로 힘을 정하고 Space나 ‘칩 던지기’도 가능합니다.','Pull and release to toss, or set power with PageUp/PageDown or the slider, then use Space or Toss chip.'));
 if(g.phase==='spinning'){
 ...
 if(g.phase==='held')return g.shotPoints+t('점 슛 · 높이 ','-point shot · Elevation ')+loft+'° · '+t('↑/↓로 높이 · PageUp/PageDown으로 힘 · Space로 슛. 칩을 당겼다 놓으면 슛, 골대 방향은 자동입니다.','Up/down sets elevation · PageUp/PageDown power · Space shoots. Drag and release also shoots; aim follows the hoop.');
...
export function curlingGuide(g,t,dragging=false,spin=0){
 ...
 return (g.shots+1)+t('/3번째 칩 · ←/→ 조준 · ↑/↓ 또는 PageUp/PageDown 힘 · Shift+←/→ 회전 · Space 발사. 드래그 중에는 ←/→로 회전. 원 3·2·1점 + 밀어낸 물건당 2점, 목표 6점.','/3: Left/right aim · Up/down or PageUp/PageDown power · Shift+left/right curl · Space launches. While dragging, left/right adjusts curl. Rings 3/2/1, cleared objects +2, target 6.');
}
