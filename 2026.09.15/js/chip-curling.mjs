...
export function mountCurling(host,{onExit,onWin,english=false}={}){
 ...
 canvas.onkeydown=e=>{if(e.key===' '){e.preventDefault();if(!e.repeat)launch();}if(['ArrowLeft','ArrowRight'].includes(e.key)){e.preventDefault();if(game.phase==='ready'){const direction=e.key==='ArrowLeft'?-1:1;if(pointer?.mode==='aim')spin.value=THREE.MathUtils.clamp(Math.round((+spin.value+direction*.1)*10)/10,-1,1);else aim.value=THREE.MathUtils.clamp(+aim.value+direction*.01,-.35,.35);}}};
