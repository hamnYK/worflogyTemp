...
export function mountCurling(host,{onExit,onWin,english=false}={}){
 ...
 let distance=30,azimuth=.4,elevation=.9,disposed=false,frame,timer,last=performance.now(),acc=0,phase='',pointer=null;
 function view(){const d=distance*Math.max(1,1.1/camera.aspect);camera.position.set(Math.sin(azimuth)*Math.cos(elevation)*d,Math.sin(elevation)*d,Math.cos(azimuth)*Math.cos(elevation)*d);camera.lookAt(0,0,0);camera.updateMatrixWorld();}
 ...
 progress.textContent=t('투구 ','SHOTS ')+game.shots+'/3 · '+game.score+t('점 / 목표 6점',' PTS / TARGET 6');
 ...
 canvas.onpointerdown=e=>{if(pointer)return;const p=world(e);pointer={id:e.pointerId,x:e.clientX,y:e.clientY,mode:e.button===0&&game.phase==='ready'&&p&&Math.hypot(p.x,p.z-7)<.8?'aim':'orbit'};canvas.setPointerCapture(e.pointerId);canvas.focus({preventScroll:true});};
 canvas.onpointermove=e=>{if(!pointer||pointer.id!==e.pointerId)return;if(pointer.mode==='aim'){const p=world(e);if(p){aim.value=THREE.MathUtils.clamp(Math.atan2(-p.x,Math.max(.1,p.z-7)),-.35,.35);power.value=THREE.MathUtils.clamp(Math.hypot(p.x,p.z-7)*2,1,7);}}else{azimuth-=(e.clientX-pointer.x)*.007;elevation=THREE.MathUtils.clamp(elevation+(e.clientY-pointer.y)*.004,.62,1.16);pointer.x=e.clientX;pointer.y=e.clientY;view();}};
 canvas.onpointerup=e=>{if(!pointer||pointer.id!==e.pointerId)return;if(pointer.mode==='aim'&&Math.hypot(e.clientX-pointer.x,e.clientY-pointer.y)>8)launch();pointer=null;};
 canvas.onpointercancel=()=>pointer=null;
 canvas.addEventListener('wheel',e=>{e.preventDefault();distance=THREE.MathUtils.clamp(distance*Math.exp(e.deltaY*.001),18,40);view();},{passive:false});
 canvas.onkeydown=e=>{if(e.key===' '){e.preventDefault();if(!e.repeat)launch();}if(['ArrowLeft','ArrowRight'].includes(e.key)){e.preventDefault();if(game.phase==='ready')aim.value=THREE.MathUtils.clamp(+aim.value+(e.key==='ArrowLeft'?-.01:.01),-.35,.35);}};
 root.querySelectorAll('[data-camera]').forEach(b=>b.onclick=()=>{const k=b.dataset.camera;if(k==='in')distance=Math.max(18,distance-2);if(k==='out')distance=Math.min(40,distance+2);if(k==='left')azimuth-=.2;if(k==='right')azimuth+=.2;if(k==='home'){distance=30;azimuth=.4;elevation=.9;}view();});
 function reset(){game.reset();phase='';result.className='chip-result';result.textContent='';aim.value=spin.value=0;power.value=4.2;status.textContent=t('칩 3개로 6점에 도전하세요. 중심부터 3·2·1점입니다.','Reach 6 points with 3 chips. Rings score 3, 2, and 1 from the center.');refresh();}
 ...
 if(phase==='won'||phase==='fail'){result.textContent=phase==='won'?'CLEAR':'RETRY';result.classList.add('show');status.textContent=game.score+t('점! ', ' points! ')+(phase==='won'?t('AFTER HOURS로 돌아갑니다.','Returning to AFTER HOURS.'):t('다시 도전합니다.','Starting another round.'));timer=setTimeout(()=>{if(!disposed){if(game.phase==='won')onWin();else reset();}},1800);}}
