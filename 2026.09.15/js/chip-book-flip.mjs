...
export function mountBookFlip(host,{onExit,onWin,english=false}={}){
 ...
 canvas.onkeydown=e=>{if(adjustPower(e,power,!!pointer))return;if(['ArrowLeft','ArrowRight','ArrowUp','ArrowDown',' '].includes(e.key)){e.preventDefault();if(game.phase!=='ready'||pendingStrike||pointer)return;if(e.key===' '){if(!e.repeat)strikeBook();}else{hit.x=THREE.MathUtils.clamp(hit.x+(e.key==='ArrowRight'?.2:e.key==='ArrowLeft'?-.2:0),-2.9,2.9);hit.z=THREE.MathUtils.clamp(hit.z+(e.key==='ArrowDown'?.2:e.key==='ArrowUp'?-.2:0),-3.9,3.9);}}};
