...
(()=>{
 ...
 function render(){
  ...
  for(const [button,offset] of [[up,-1],[down,1]]){
   ...
   button.setAttribute('aria-label',label);button.title=label;if(!button.disabled)button.setAttribute('aria-controls',sections[index+offset].id);else button.removeAttribute('aria-controls');
  }
